...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-3):
	...
	for i in range(1000):
		...
		for state in range(nS):
			...
			# print(P[state][policy[state]])
			for probability, nextstate, reward, terminal in P[state][policy[state]]:
				v_new[state] = probability*(reward+gamma*v_new[nextstate])
				
				if terminal:
					v_new[state] = reward

		if(np.all(np.abs(value-v_new)<tol)):
			break	
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
	...
	# YOUR IMPLEMENTATION HERE #
	for state in range(nS):
		valMax = value_from_policy[state]
		for action in range(nA):
			probability, nextstate, reward, terminal = P[state][action][0]
			Qval = probability * (reward + gamma * value_from_policy[nextstate])
			if Qval > valMax:
...
def policy_iteration(P, nS, nA, gamma=0.9, tol=10e-3):
	...
	for i in range(1000):
		...
		# if  np.all(np.abs(value_function - v_new) < tol):
		# 	print('tatti')
		# 	break
...
def value_iteration(P, nS, nA, gamma=0.9, tol=1e-3):
	...
	"""
	# print(P)
	value_function = np.zeros(nS,dtype='double')
	policy = np.zeros(nS, dtype=int)
	...
	for i in range(1000):
		...
		for s in range(nS):
			...
			for a in range(nA):
				...
				action_reward.append(q)
			v_new[s]= np.max(action_reward)
